...
const PROVIDER_TAKEOVER_PATCH_MARKER = '[codex-provider-switcher:provider-takeover-v7]';
const SIXTH_PROVIDER_TAKEOVER_PATCH_MARKER = '[codex-provider-switcher:provider-takeover-v6]';
...
function inspectResumeParameterSource(source, paramsSource, beforeIndex) {
  if (paramsSource.startsWith('{') && paramsSource.endsWith('}')) {
    if (paramsSource.includes('${')) {
      return { error: 'thread/resume 参数对象无法安全复用' };
    }
    return parseResumeParameters(paramsSource);
  }
  if (!new RegExp(`^${IDENTIFIER}$`).test(paramsSource)) {
    return { error: 'thread/resume 参数对象无法安全复用' };
  }

  const searchStart = Math.max(0, beforeIndex - 16000);
  const requestDeclarationNeedle = `let ${paramsSource}=`;
  const requestDeclarationIndex = source.lastIndexOf(requestDeclarationNeedle, beforeIndex);
  if (requestDeclarationIndex < searchStart) {
    return { error: 'thread/resume 参数变量来源无法安全验证' };
  }
  const requestDeclarationMatch = new RegExp(
    `^let ${escapeRegExp(paramsSource)}=(${IDENTIFIER})\\.completeRequest\\((${IDENTIFIER})\\)(?=[,;])`
  ).exec(source.slice(requestDeclarationIndex));
  if (!requestDeclarationMatch) {
    return { error: 'thread/resume 参数变量来源无法安全验证' };
  }
  const requestAssignments = [
    ...source.slice(requestDeclarationIndex, beforeIndex).matchAll(
      new RegExp(`\\b${escapeRegExp(paramsSource)}=(?!=)`, 'g')
    )
  ];
  if (requestAssignments.length !== 1) {
    return { error: 'thread/resume 参数变量在调用前被重新赋值' };
  }

  const baseParamsVar = requestDeclarationMatch[2];
  const baseDeclarationNeedle = `let ${baseParamsVar}={`;
  const baseDeclarationIndex = source.lastIndexOf(baseDeclarationNeedle, requestDeclarationIndex);
  if (baseDeclarationIndex < searchStart) {
    return { error: 'thread/resume 基础参数对象来源无法安全验证' };
  }
  const baseParamsStart = baseDeclarationIndex + baseDeclarationNeedle.length - 1;
  const baseParamsEnd = scanDelimited(source, baseParamsStart, '{', '}');
  if (
    baseParamsEnd < 0
    || baseParamsEnd >= requestDeclarationIndex
    || ![';', ','].includes(source[baseParamsEnd + 1])
  ) {
    return { error: 'thread/resume 基础参数对象边界不完整' };
  }
  const baseAssignments = [
    ...source.slice(baseDeclarationIndex, requestDeclarationIndex).matchAll(
      new RegExp(`\\b${escapeRegExp(baseParamsVar)}=(?!=)`, 'g')
    )
  ];
  if (baseAssignments.length !== 1) {
    return { error: 'thread/resume 基础参数对象在完成前被重新赋值' };
  }
  const sensitiveMutationPattern = new RegExp(
    `\\b${escapeRegExp(baseParamsVar)}\\.(?:threadId|modelProvider)\\s*=`
  );
  if (sensitiveMutationPattern.test(source.slice(baseParamsEnd + 1, requestDeclarationIndex))) {
    return { error: 'thread/resume 关键基础参数在完成前被改写' };
  }

  const baseParamsSource = source.slice(baseParamsStart, baseParamsEnd + 1);
  if (baseParamsSource.includes('${')) {
    return { error: 'thread/resume 基础参数对象无法安全复用' };
  }
  return parseResumeParameters(baseParamsSource);
}
...
function buildSixthResumeThunk(options) {
  return buildFifthResumeThunk(options)
    .split(FIFTH_PROVIDER_TAKEOVER_PATCH_MARKER)
    .join(SIXTH_PROVIDER_TAKEOVER_PATCH_MARKER)
    .split(`${SIXTH_PROVIDER_TAKEOVER_PATCH_MARKER} already has an active writer; `)
    .join(`${SIXTH_PROVIDER_TAKEOVER_PATCH_MARKER} ${PROVIDER_TAKEOVER_BLOCK_MARKER} `);
}

function buildResumeThunk(options) {
  const sixthThunk = buildSixthResumeThunk(options)
    .split(SIXTH_PROVIDER_TAKEOVER_PATCH_MARKER)
    .join(PROVIDER_TAKEOVER_PATCH_MARKER);
  const oldReadError = `current ChatGPT account state could not be read; sending is blocked`;
  const newReadError = `current OpenAI authentication state could not be read; sending is blocked`;
  const oldAccountGuard = `if(codexProviderSwitcherAccountReadResult?.account?.type!==\`chatgpt\`)throw Error(\`${PROVIDER_TAKEOVER_PATCH_MARKER} ${PROVIDER_TAKEOVER_BLOCK_MARKER} no ChatGPT account is signed in; sending is blocked\`);`;
  const newAccountGuard = [
    'let codexProviderSwitcherOpenAIAccountType=codexProviderSwitcherAccountReadResult?.account?.type??null;',
    `if(codexProviderSwitcherOpenAIAccountType===null)throw Error(\`${PROVIDER_TAKEOVER_PATCH_MARKER} ${PROVIDER_TAKEOVER_BLOCK_MARKER} effective provider is openai but no OpenAI login is active; sign in with ChatGPT or an OpenAI API key, or switch to the custom API and reload; sending is blocked\`);`,
    `if(![\`chatgpt\`,\`apiKey\`].includes(codexProviderSwitcherOpenAIAccountType))throw Error(\`${PROVIDER_TAKEOVER_PATCH_MARKER} ${PROVIDER_TAKEOVER_BLOCK_MARKER} unsupported OpenAI login type; sending is blocked\`);`
  ].join('');
  if (!sixthThunk.includes(oldReadError) || !sixthThunk.includes(oldAccountGuard)) {
    throw new Error('Provider 接管认证守卫模板无法升级到 v7');
  }
  return sixthThunk
    .replace(oldReadError, newReadError)
    .replace(oldAccountGuard, newAccountGuard);
}
...
function inspectOriginalResume(source) {
  ...
  const callBoundary = closeParenthesis < 0 ? '' : source[closeParenthesis + 1];
  const hasSafeFollowingDeclarator = callBoundary === ',' && new RegExp(
    `^,\\s*${IDENTIFIER}\\s*=`
  ).test(source.slice(closeParenthesis + 1));
  if (closeParenthesis < 0 || (callBoundary !== ';' && !hasSafeFollowingDeclarator)) {
  ...
  const parsedParams = inspectResumeParameterSource(source, paramsSource, statementStart);
...
function inspectPatchedProviderTakeoverSource(
  ...
  if (source[paramsStart] === '{') {
    paramsEnd = scanDelimited(source, paramsStart, '{', '}');
  } else {
    const paramsIdentifier = new RegExp(`^${IDENTIFIER}`).exec(source.slice(paramsStart));
    paramsEnd = paramsIdentifier ? paramsStart + paramsIdentifier[0].length - 1 : -1;
  }
  if (paramsEnd < 0 || source[paramsEnd + 1] !== ';') {
    return null;
  }
  const paramsSource = source.slice(paramsStart, paramsEnd + 1);
  const parsedParams = inspectResumeParameterSource(source, paramsSource, wrapperStart);
  if (parsedParams.error) {
    return null;
  }
...
function upgradeSixthProviderTakeoverSource(source) {
  return upgradeProviderTakeoverSource(
    source,
    SIXTH_PROVIDER_TAKEOVER_PATCH_MARKER,
    buildSixthResumeThunk
  );
}
...
function patchProviderTakeoverSource(source) {
  ...
  const hasSixthPatch = source.includes(SIXTH_PROVIDER_TAKEOVER_PATCH_MARKER);
  ...
  if (hasSixthPatch) {
    const upgradedSource = upgradeSixthProviderTakeoverSource(source);
