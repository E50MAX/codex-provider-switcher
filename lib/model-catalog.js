...
function resolveReasoningEffort(model, existing) {
  ...
  for (const candidate of candidates) {
    const normalized = typeof candidate === 'string' ? candidate.toLowerCase() : undefined;
...
function resolveAccountSelection(cache, existing = {}) {
  ...
  const reviewModel = models.some((model) => model.slug === existing.reviewModel)
    ? existing.reviewModel
    : selectedModel.slug;
  const modelReasoningEffort = resolveReasoningEffort(selectedModel, existing);
  const providerSpecificReasoningEffort = resolveReasoningEffort(selectedModel, {
  ...
  return {
    ...
    adjusted: selection.model !== existing.model
      ...
      || providerSpecificReasoningEffort !== existing.modelReasoningEffort
