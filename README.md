...
- **切换很直接**：点击 VS Code 右下角的 `Codex 当前: 账户` 或 `Codex 当前: API`，选择连接并重载窗口。
- **配置只填两项**：第一次使用自定义 API 时，按提示填写可信服务商提供的 **HTTPS Base URL** 和 **API Key**；模型与推理等级仍在官方 Codex 输入框下方选择。
- **本地历史接着用**：账户和自定义 API 可以显示同一份本地历史列表，旧会话保留原 `thread ID` 和内容，不需要导出或复制。
...
安装前需要先安装并登录发布者为 **OpenAI** 的官方 Codex 扩展。首次完整配置、共享历史与旧会话接管仍请按下面的顺序操作；继续旧会话前，也请确认当前 Provider 是你信任的服务。
...
> 在旧会话发送下一条消息时，该会话保留的提示词、源码上下文、图片引用和工具输出可能会提供给新 Provider 或新登录账户。这是一次新的隐私与信任边界，请只切换到你信任的服务。若会话仍活动或任何校验失败，扩展都会保持输入框只读。2.3.9 会区分真正的 writer conflict 与 Provider、模型、登录态等安全校验失败，并为后者显示具体原因。
...
2.3.9 起，真正的 writer conflict 仍会自动等待最多约 10 秒，让重载前的 App Server 释放写锁；持续冲突会保留官方占用提示。配置、模型目录、登录态或运行时选择校验失败时，则会显示 `Provider Switcher 已阻止恢复此对话`，并弹出包含具体原因的错误通知。请按通知修复后点击重试；不要删除会话文件、数据库或 writer-lock 文件。
