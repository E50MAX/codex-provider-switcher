...
这是一个仅限**原生 Windows 本地 VS Code 窗口**的扩展，用来在官方 Codex 的两种连接之间切换：

- OpenAI 官方连接（ChatGPT 账户或 OpenAI API Key 登录）；
...
- **切换很直接**：点击 VS Code 右下角的 `Codex 当前: OpenAI` 或 `Codex 当前: API`，选择连接并重载窗口。
- **配置只填两项**：第一次使用自定义 API 时，按提示填写可信服务商提供的 **HTTPS Base URL** 和 **API Key**；模型与推理等级仍在官方 Codex 输入框下方选择。
- **本地历史接着用**：OpenAI 官方连接和自定义 API 可以显示同一份本地历史列表，旧会话保留原 `thread ID` 和内容，不需要导出或复制。
- **随时可以切回**：切换器不会注销 ChatGPT，也不会删除官方 Codex 的登录信息。

最快安装方式：在 VS Code 扩展商店搜索 `Secure Codex Provider Switcher`，确认发布者为 `e50max`；也可以直接运行：

```powershell
code --install-extension e50max.codex-provider-switcher
```

安装前需要先安装发布者为 **OpenAI** 的官方 Codex 扩展。首次完整配置仍需要先通过 ChatGPT 账户或 OpenAI API Key 登录一次，让官方 Codex 生成模型缓存；共享历史与旧会话接管请按下面的顺序操作。继续旧会话前，也请确认当前 Provider 是你信任的服务。
...
4. 打开官方 Codex 面板，按提示使用 ChatGPT 账户或 OpenAI API Key 登录。
...
1. 等当前正在生成的回复结束。
2. 仔细阅读“让当前连接接管 OpenAI / 自定义 API 的旧会话？”提示。
3. 点击 **启用旧会话切换并重载**。
4. 重载后确认右下角显示 `Codex 当前: OpenAI` 或 `Codex 当前: API`。

暂时不启用可点击 **Cancel**。以后按 `Ctrl+Shift+P`，运行 `Codex: 修复旧会话 Provider 接管`。

这项修复会在明确同意后修改官方 Codex 的三处本地 webview 资源：会话恢复调用、恢复错误通知和输入框阻断提示。恢复旧会话时，它先通过只读的 `config/read` 取得当前工作目录下的有效配置，再用 `model/list` 核对当前 Provider 真正可用的模型、推理等级和服务档；OpenAI 官方连接还会用 `account/read`（`refreshToken: false`）确认当前已通过 ChatGPT 或 OpenAI API Key 登录。兼容的会话专属选择会保留；切换账户或 API 后已不兼容的选择会依次回退到当前配置或模型默认值。随后它把完整选择明确传给 `thread/resume`，对窗口重载交接造成的短暂写锁做最多约 10 秒的有限重试，并核对同一 `thread ID` 和实际运行时选择。
...
> 在旧会话发送下一条消息时，该会话保留的提示词、源码上下文、图片引用和工具输出可能会提供给新 Provider 或新登录账户。这是一次新的隐私与信任边界，请只切换到你信任的服务。若会话仍活动或任何校验失败，扩展都会保持输入框只读。2.3.9 会区分真正的 writer conflict 与 Provider、模型、登录态等安全校验失败；2.3.10 同时接受 ChatGPT 与 OpenAI API Key 两种官方本地登录方式，并继续阻断未登录或未知认证类型。
...
- 点击 VS Code **右下角状态栏**里的 `Codex 当前: OpenAI` 或 `Codex 当前: API`；
- 按 `Ctrl+Shift+P`，输入并运行 `Codex: 切换 OpenAI / 自定义 API`。
...
配置保存后，如果当前仍是 OpenAI 官方连接，扩展会明确提示 **当前连接仍为 OpenAI 官方连接**：
...
如果正在使用自定义 API 时更新 Base URL 或 API Key，本扩展会把它视为一次真实连接变更：必须先确认重载，确认后才保存并立即重载；取消确认时不会写入新的地址或密钥。

## 八、切回 OpenAI 官方连接

1. 点击 VS Code 右下角的 `Codex 当前: API`。
2. 在菜单中点击 **OpenAI 官方连接**。
3. 等当前回复结束，再让 VS Code 自动重载。
4. 重载后确认右下角显示 `Codex 当前: OpenAI`；新对话和重新恢复的空闲旧会话都会使用该连接。

切换器不会注销 ChatGPT，也不会删除官方 Codex 的 OpenAI API Key 登录信息。以后可以用同一个状态栏入口来回切换；如果当前未登录，官方 Codex 会要求先使用 ChatGPT 或 OpenAI API Key 登录。

如果你在官方 Codex 中主动退出并登录另一个 ChatGPT 账户，本地历史仍属于当前 Windows 用户的同一个 `CODEX_HOME`，不会按 ChatGPT 身份隔离。新账户能看到共享列表里的本地会话；切换器只确认当前存在受支持的 OpenAI 登录（ChatGPT 或 OpenAI API Key），不会读取邮箱，也不会把旧 thread 绑定到原账户身份。2.3.4 会移除新账户模型目录不支持的旧账户配置，但不会隐藏旧账户留下的本地内容。

## 常见问题

### 提示找不到 `models_cache.json`

第一次配置时，官方 Codex 还没有生成模型缓存。先切回 OpenAI 官方连接，通过 ChatGPT 账户或 OpenAI API Key 登录官方 Codex，打开一次新对话并展开一次模型菜单，然后重载窗口，再重新配置自定义 API。成功生成过中转模型目录后，2.3.4 会在新账户缓存暂时缺少 Sol、Terra 或 Luna 时继续使用这份已经校验并保存在本机的目录。

### 状态栏提示“环境不受支持”

当前版本只支持原生 Windows 本地窗口。若窗口处于 WSL、SSH、Dev Container 等 VS Code Remote 环境，或官方 Codex 启用了 `chatgpt.runCodexInWindowsSubsystemForLinux`，官方 Codex 会在另一套 Linux/远程文件系统中运行；Windows 侧的 `CODEX_HOME` 与 DPAPI 密钥命令不能被安全地假定为同一运行环境。切换器会在创建目录、写配置或修改官方扩展前停止。请回到本地 Windows 窗口，或关闭 Codex 的 WSL 运行设置并重载。参见 [OpenAI Codex on Windows / WSL](https://learn.chatgpt.com/docs/windows/wsl)。

### 状态栏显示 `Codex 默认: 其他`

这表示 `config.toml` 当前选择了本扩展不管理的第三方 Provider。切换器会保留该事实，不会把它误报成 OpenAI 官方连接；点击状态栏后可明确选择 OpenAI 官方连接或本扩展管理的自定义 API。有效 TOML 的单引号字符串和行尾 `#` 注释均可识别，重复或无效的顶层 Provider 配置会拒绝修改。
...
2.3.9 起，真正的 writer conflict 仍会自动等待最多约 10 秒，让重载前的 App Server 释放写锁；持续冲突会保留官方占用提示。配置、模型目录、登录态或运行时选择校验失败时，则会显示 `Provider Switcher 已阻止恢复此对话`，并弹出包含具体原因的错误通知。2.3.10 若发现有效 Provider 仍是 `openai` 且没有受支持的 OpenAI 登录，会明确提示登录 ChatGPT / OpenAI API Key，或先运行 `Codex: 使用自定义 API` 并重载。请按通知修复后点击重试；不要删除会话文件、数据库或 writer-lock 文件。
...
按 `Ctrl+Shift+P`，运行 `Codex: 修复 OpenAI / 自定义 API 共享历史`，同意修复并重载窗口。如果扩展提示当前官方 Codex 版本不受支持，它会保持所有文件不变；请先升级本切换器，再查看 GitHub Release 说明。

共享历史不会恢复已经删除的会话，也不会拉取其他电脑或 ChatGPT 云端的聊天记录。

### 退出后登录另一个 ChatGPT 账户，会读错历史吗

不会把请求误发给已经退出的账户：OpenAI 官方连接恢复旧会话前会用不刷新 token 的 `account/read` 确认当前已通过 ChatGPT 或 OpenAI API Key 登录；未登录、未知认证类型或登录状态无法读取时会阻止发送。新账户可用模型不同也不会沿用不兼容选择，旧配置会回退到新账户当前支持的值。
...
如果自定义 API 正在使用，确认按钮会显示 **删除密钥并重载**。扩展会在删除后立即重载，避免已经启动的 Codex 运行时继续持有旧密钥；重载前状态栏会明确显示 `Codex API: 缺少密钥`。
...
OpenAI App Server 的 `thread/resume` 支持指定 `modelProvider`、模型、服务档和配置覆盖，`config/read` 可返回配置分层后的当前有效配置，`model/list` 可返回当前可用模型及其推理等级。`thread/unsubscribe` 只移除当前连接的订阅；最后一个订阅者离开后，服务器仍会让 thread 在约 30 分钟的空闲宽限期内保持 loaded，不能把取消订阅当成立即卸载。2.3.4 会在每次恢复前解析并验证完整运行时选择，OpenAI 官方连接还会检查当前登录态；2.3.10 接受 App Server 返回的 `chatgpt` 与 `apiKey` 两种本地认证类型。它只对窗口重载期间真实返回的短暂 writer conflict 做有限重试，成功后再核对 thread 身份和 App Server 实际返回的 Provider、模型、推理等级与服务档。若已加载运行时仍是另一套选择，则安全阻止发送并要求重载。参见 [OpenAI Codex App Server](https://learn.chatgpt.com/docs/app-server) 和 [OpenAI Authentication](https://learn.chatgpt.com/docs/auth)。
...
- 旧会话接管只修改唯一且经过验证的恢复调用及其错误通知、输入框阻断提示；仅对官方 writer conflict 做有上限的等待重试，并用独立标记呈现 Provider Switcher 自身的安全阻断原因。已加载的 Provider 不匹配时只取消本连接的订阅并拒绝放行，不在 30 分钟空闲宽限期内立即重试；
...
- 正在使用的自定义 API 地址或密钥发生变化时，必须确认并完成窗口重载；取消确认不会保存新连接信息。
- 同一窗口内的连接操作会串行执行；若 `config.toml` 在确认期间被其他程序修改，本次写入会被拒绝而不会覆盖外部改动。
- VS Code Remote 和 Codex WSL 运行模式会在任何文件修改前被拒绝，避免写入错误的 `CODEX_HOME`。
...
- Windows 10/11 的原生本地 VS Code 窗口（不支持 VS Code Remote，也不支持 Codex 的 WSL 运行模式）；
- VS Code 1.96 或更高版本；
